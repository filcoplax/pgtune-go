...
func main() {
	...
	workMemValue = totalMemoryInKb / 2 / (FinalConnectionNum * 2)
	...
	if DBVersionFloat >= 9.5 && *CPUNum >= 4 {
		fmt.Println("max_worker_processes", "=", *CPUNum)

		if DBVersionFloat >= 9.6 {
			fmt.Println("max_parallel_workers_per_gather", "=", 4)
		}

		if DBVersionFloat >= 10 {
			fmt.Println("max_parallel_workers", "=", *CPUNum)
			fmt.Println("max_parallel_maintenance_workers", "=", 4)
		}
	}
